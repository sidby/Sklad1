...
$.jgrid.extend({
	...
	groupingCalculations : {
		handler: function(fn, v, field, round, roundType, rc) {
			var funcs = {
			    sum: function () {
			        // Sergei Lobachov (sid.by): function parseFloat() will only consider the parts of 
			        // the string up until in reaches a non +, -, number, exponent or decimal point. 
			        //Once it sees the comma it stops looking
			        return ((parseFloat((v + '').replace(",", ".") || 0) + parseFloat((rc[field] + '').replace(",", ".") || 0)).toFixed(2));
				},
